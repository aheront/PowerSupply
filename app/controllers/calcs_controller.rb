class CalcsController < ApplicationController
  ...
  def create
    ...
    @math = Motherboard.find(calc_params[:motherboard_id])
    ...
    @calc = (@cpu.power + (@vga.vga_power*@vga_count) + (@ram.power_ram * @ram_count) + (@fan_count*3) + @wat.water_power + @math.power + (@sata_count * 25)) * 1.3
  end
  private
  def calc_params
